...
### TABLES ------------ updated 4-30-23 / 8-25-23
# table 4, im-ess ---------------
...
#d1f<-c(361,359,359,360,354,354,355,355) # I think this is dependent on parameters etc. 
...
base23f<-c(B$Catches,B$Index,B$Size_comp_like,B$Recruit_pen,B$MeanF_pen, B$M_devs, B$Sex_ratio,sum(B$priorDensity),B$spr_bmsy,B$spr_bmsy*B$spr_depl,B$sd_fmsy[1],B$sd_fofl[1],B$spr_cofl,B$spr_cofl*0.8,B$survey_q[1])
...
m22<-c(C$Catches,C$Index,C$Size_comp_like,C$Recruit_pen,C$MeanF_pen, C$M_devs, C$Sex_ratio,sum(C$priorDensity),C$spr_bmsy,C$spr_bmsy*C$spr_depl,C$sd_fmsy[1],C$sd_fofl[1],C$spr_cofl,C$spr_cofl*0.8,C$survey_q[1])
...
d9m<-cbind(d3f_names,base23f, m230a, m22)
#d1m<-d5m-d1m
#d2m<-d5m-d2m
#d3m<-d5m-d3m
#d4m<-d5m-d4m
...
write.csv(d9m,"./BBRKC/bbrkc_23f/doc/tables/likelihood.csv", row.names = FALSE)
