...
folder <- "bbrkc_23f/model_22/retrospective" # update annually 
...
mod_names <- c("22.0 (2023)", "2022", "2021", "2020", "2019", "2018", "2017", "2016", "2015", "2014", "2013") # , "2012")
# add current year and last year here and delete 1 older year
#mod_names <- c("16.0 (2019)", "16.0 (2020)", "16.0a (fix R ter)", "20.1 (no pot)")
.MODELDIR = c(paste0(here::here(), "/BBRKC/bbrkc_23f/model_22/"),
...
.FILES     = c("./BBRKC/bbrkc_23f/model_22/retrospective/")
...
ssb2 %>% 
  ...
  ylab("Mature male biomass (1000 tons) on Feb 15th") +
...
ssb2 %>% 
  mutate(ssb = ssb/1000) %>% 
  mutate(model.end.yr = ifelse(Model == '22.0 (2023)', 2022, as.integer(Model)-1)) %>% # change to current model name and then one year less
