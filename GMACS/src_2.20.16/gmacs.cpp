...
WriteFileName(TheHeader);
  ...
  weightunit.allocate("weightunit");
  numbersunit.allocate("numbersunit");
  StockName.allocate("StockName");
...
WriteFileName(StockName);
 ...
 WriteFileName(sdJitter);
  OutRefPars.allocate("OutRefPars");
 WriteFileName(OutRefPars);
  OutRecruit.allocate("OutRecruit");
 WriteFileName(OutRecruit);
  OutSSB.allocate("OutSSB");
 WriteFileName(OutSSB);
  Outfbar.allocate("Outfbar");
 WriteFileName(Outfbar);
  OutDynB0.allocate("OutDynB0");
 WriteFileName(OutDynB0);
  nyrRetro.allocate("nyrRetro");
  if (nyrRetro1 > -1) nyrRetro = nyrRetro1;//--override value from gmacs.dat
  WriteFileName(nyrRetro);
  TurnOffPhase.allocate("TurnOffPhase");
  StopAfterFnCall.allocate("StopAfterFnCall");
  CalcRefPoints.allocate("CalcRefPoints");
  usepinfile.allocate("usepinfile");
...
echoinput << verbose << " # VERBOSE FLAG (0 = off, 1 = on, 2 = objective func; 3 diagnostics)" << endl;
 ...
 gmacs_data << nCatchDF << "  #--Number of catch data frames" << endl;
  nCatchRows.allocate(1,nCatchDF);
  dCatchData.allocate(1,nCatchDF);
float TempW;
float SDlogD;
    {
  ...
  fhitfut.allocate(1,nseason,1,nfleet);
  nFparams.initialize();
  nYparams.initialize();
  fhit.initialize();
  yhit.initialize();
...
gmacs_data << fmtRAD    << "  #--input format type (0: old format, 1: new format)" << endl;
 ...
 gmacs_data << nSurveys << "  #--Number of dataframes" << endl; //TODO: change name to nSurveyDFs
  ...
  cpue_time.allocate(1,nSurveyRows);
 ...
 gmacs_data << nSizeComps_in << "   #--number of dataframes" << endl;
  nSizeCompRows_in.allocate(1,nSizeComps_in);
  nSizeCompCols_in.allocate(1,nSizeComps_in);
  d3_SizeComps_in.allocate(1,nSizeComps_in);
  {
    cifstream* pGD = ad_comm::global_datafile;
    if (fmtZCs_in==0){
...
if(GrowthObsType==GROWTHCLASS_VALS) gmacs_data << "## size-class-at-release, sex, size-class-at-recapture, and time-at-liberty fleet recapture_year number" << endl;
 ...
 gmacs_data << endl << "# Environmental data" << endl;
  fmtEIs_in.allocate("fmtEIs_in");
 ECHO(fmtEIs_in)
 gmacs_data << fmtEIs_in << "   #--format flag (0=old style, 1=new style)" << endl;
  NenvIndics.allocate("NenvIndics");
 ECHO(NenvIndics)
 gmacs_data << NenvIndics << "   #--number of environmental indices " << endl;
  EnvYrs.allocate(1,NenvIndics,1,2);
  strEIs.allocate();
  EnvDataInp.allocate();
  EnvData.allocate(syr,nyr,1,NenvIndics);
  if (NenvIndics>0) {
 ...
 cout << "end of data section" << endl;
  isizeTrans_pass.allocate(1,nsex);
 ...
 echoinput << "# The blocks" << endl;
  blocklimits.allocate(0,nblocks,0,maxblocks,1,2);
 ...
 PPnt;
 n_devvars=0;
  devpoints.allocate(1,100,1,10);
  rdevpoints.allocate(1,100,1,5);
 devpoints.initialize();
 cout << " * General controls" << endl;
 ECHOSTR(" * General controls");
  model_controls.allocate(1,17,"model_controls");
  Rec_bias.allocate(syr,nyr);
  {
 ...
 for (int i=1;i<=ntheta;i++) {
  ...
  prior_theta_type.allocate(1,ntheta);
  prior_theta_p1.allocate(1,ntheta);
  prior_theta_p2.allocate(1,ntheta);
 ...
 if (lw_type < 1 || lw_type > 3)
  ...
  ECHO(lw_dim2);
  lw_alfa.allocate(1,nsex,1,nmature);
  lw_beta.allocate(1,nsex,1,nmature);
  mean_wt_in.allocate(1,lw_dim,1,nclass);
  mean_wt.allocate(1,nsex,1,nmature,syr,nyrRetro+1,1,nclass);
 ...
 ECHO(nSizeClassRec );
  use_func_mat.allocate("use_func_mat");
 gmacs_ctl << "# Use functional maturity for terminally molting animals (0=no; 1=Yes)?" << endl;
 gmacs_ctl << use_func_mat << endl;
 ECHO(use_func_mat);
  bUseCustomGrowthMatrix2.allocate(1,nsex);
  bUseGrowthIncrementModel2.allocate(1,nsex);
  bUseCustomMoltProbability2.allocate(1,nsex);
  bUseCustomMatureProbability2.allocate(1,nsex);
 ...
 bUseCustomMatureProbability2.initialize();
  Growth_controls.allocate(1,3*nsex,1,3);
 ...
 if (nmature==2)
  ...
  nparGs.allocate(1,3*nsex);
  G_block.allocate(1,3*nsex);
  G_RW_sigma.allocate(1,3*nsex);
  ...
  GrowPnt.allocate(1,3*nsex*Nyears,1,nclass+5);
  Gpar_control.allocate(1,1000,1,15);
  GtoIG.allocate(1,1000,1,4);
 PPstoreG = PPnt;
  {
   n_Gpar = 0;
  ...
  echoinput <<  GrowPnt << endl;
  iYrsIncChanges.allocate(1,nsex,syr,nyrRetro);
  iYrsMoltChanges.allocate(1,nsex,syr,nyrRetro);
  iYrsMatureChanges.allocate(1,nsex,syr,nyrRetro);
  nSizeIncVaries.allocate(1,nsex);
  nMoltVaries.allocate(1,nsex);
  nMatureVaries.allocate(1,nsex);
  int inew;
  ...
  G_ival.allocate(1,n_Gpar);
  G_lb.allocate(1,n_Gpar);
  G_ub.allocate(1,n_Gpar);
  G_phz.allocate(1,n_Gpar);
  prior_Gtype.allocate(1,n_Gpar);
  prior_G_p1.allocate(1,n_Gpar);
  prior_G_p2.allocate(1,n_Gpar);
  G_relative.allocate(1,n_Gpar);
...
for (int ig=1;ig<=nsex;ig++)
 if (bUseCustomGrowthMatrix2(ig)==GROWTH_FIXEDGROWTHTRANS || bUseCustomGrowthMatrix2(ig)==GROWTH_FIXEDSIZETRANS)
  ...
  CustomMoltProbabilityMatrix.allocate(1,nsex,1,maxMoltVaries,1,nclass);
 for (int ig=1;ig<=nsex;ig++)
  ...
  CustomMatureProbabilityMatrix.allocate(1,nsex,1,maxMatureVaries,1,nclass);
 for (int ig=1;ig<=nsex;ig++)
 ...
 ECHOSTR(" * Natural mortality parameter controls");
  M_controls.allocate(1,nmature*nsex,1,13,"M_controls");
 ECHO(M_controls);
 ...
 for (int b=1; b<=nmature*nsex;b++)
  ...
  M_relative.allocate(1,nmature*nsex);
  M_type.allocate(1,nmature*nsex);
  M_extra.allocate(1,nmature*nsex);
  M_size_breakpnts.allocate(1,nmature*nsex);
  M_mirror.allocate(1,nmature*nsex);
  M_block.allocate(1,nmature*nsex);
  M_block_fn.allocate(1,nmature*nsex);
  M_env_link.allocate(1,nmature*nsex);
  M_env_var.allocate(1,nmature*nsex);
  M_RW.allocate(1,nmature*nsex);
  M_RW_blk.allocate(1,nmature*nsex);
  M_RW_sigma.allocate(1,nmature*nsex);
  M_mirror_RW.allocate(1,nmature*nsex);
 PPstoreM = PPnt;
  ...
  Mbreaks.allocate(1,nsex*nmature,1,MaxMbreaks);
 if (MaxMbreaks > 0) gmacs_ctl <<"# MaxMbreaks" << endl;
  ...
  MToIg.allocate(1,n_Mpar);
  MType.allocate(1,n_Mpar);
  MPoint.allocate(1,nsex*nmature);
  {
   int iq; iq = 0;
   for (h=1;h<=nsex;h++)
    for (m=1;m<=nmature;m++)
  ...
  M_pars.allocate(1,n_Mpar,1,7,"M_pars");
  M_ival.allocate(1,n_Mpar);
  M_lb.allocate(1,n_Mpar);
  M_ub.allocate(1,n_Mpar);
  M_phz.allocate(1,n_Mpar);
  prior_Mtype.allocate(1,n_Mpar);
  prior_M_p1.allocate(1,n_Mpar);
  prior_M_p2.allocate(1,n_Mpar);
  gmacs_ctl << endl;
 ...
 cout << " * Selectivity parameter controls" << endl;
  slx_bsex_in.allocate(1,2,1,nfleet);
  slx_type_in.allocate(1,2*nsex,1,nfleet);
  slx_include_in.allocate(1,2,1,nfleet);
  slx_extra_in.allocate(1,2*nsex,1,nfleet);
  slx_max_at_1_in.allocate(1,2*nsex,1,nfleet);
  slx_1_at_size.allocate(1,2*nsex,1,nfleet);
  slx_nret.allocate(1,2*nsex,1,nfleet);
  ret_max_in.allocate(1,2*nsex,1,nfleet);
  slx_type_npar.allocate(1,2*nsex,1,nfleet);
 Nyears3 = 2*nfleet*nsex*(nyr+1-syr+1);
  CustomSelex.allocate(1,Nyears3,1,nclass);
  gmacs_ctl << endl;
  gmacs_ctl <<"## ==================================================================================== ##" << endl;
  ...
  *(ad_comm::global_datafile) >> slx_bsex_in(2);
  ...
  gmacs_ctl << endl;
  nparSs.allocate(1,2,1,2*nfleet);
  ...
  SelPnt.allocate(1,2*nsex*nfleet*Nyears2,1,nclass+6);
  Spar_control.allocate(1,1000,1,15);
  StoIG.allocate(1,1000,1,4);
 PPstoreS = PPnt;
  {
   n_Spar = 0;
  ...
  iYrsSelChanges.allocate(1,2*nsex*nfleet,syr,nyrRetro+1);
  sel_patterns.allocate(1,1000);
  sel_patterns2.allocate(1,1000);
  nSelVaries.allocate(1,2*nsex*nfleet);
 {
  int inew;
  nsel_patterns = 0;
  ...
  S_ival.allocate(1,n_Spar);
  S_lb.allocate(1,n_Spar);
  S_ub.allocate(1,n_Spar);
  S_phz.allocate(1,n_Spar);
  prior_Stype.allocate(1,n_Spar);
  prior_S_p1.allocate(1,n_Spar);
  prior_S_p2.allocate(1,n_Spar);
  S_relative.allocate(1,n_Spar);
...
for (int it=1;it<=2;it++)
 ...
 ECHO(nSurveys);
  ...
  q_mirror.allocate(1,nSurveys);
  q_env_var.allocate(1,nSurveys);
  q_block.allocate(1,nSurveys);
  q_env_link.allocate(1,nSurveys);
  q_RW.allocate(1,nSurveys);
  q_RW_blk.allocate(1,nSurveys);
  q_RW_sigma.allocate(1,nSurveys);
  ...
  qToSurv.allocate(1,n_qpar);
  qType.allocate(1,n_qpar);
  qPoint.allocate(1,n_qpar);
  int iq; iq = 0;
  for (int ii=1;ii<=nSurveys;ii++) 
   if (q_mirror(ii)==0)
    {
  ...
  ECHO(qToSurv);
  q_pars.allocate(1,n_qpar,1,7,"q_pars");
 ...
 echoinput << q_pars << endl;
  q_ival.allocate(1,n_qpar);
  q_lb.allocate(1,n_qpar);
  q_ub.allocate(1,n_qpar);
  q_phz.allocate(1,n_qpar);
  prior_qtype.allocate(1,n_qpar);
  prior_q_p1.allocate(1,n_qpar);
  prior_q_p2.allocate(1,n_qpar);
 ...
 ECHOSTR(" * Additional CV controls");
  add_cv_controls.allocate(1,nSurveys,1,7,"add_cv_controls");
 ...
 echoinput << add_cv_controls << endl;
  add_cv_mirror.allocate(1,nSurveys);
  add_cv_block.allocate(1,nSurveys);
  add_cv_env_link.allocate(1,nSurveys);
  add_cv_env_var.allocate(1,nSurveys);
  add_cv_RW.allocate(1,nSurveys);
  add_cv_RW_blk.allocate(1,nSurveys);
  add_cv_RW_sigma.allocate(1,nSurveys);
  ...
  ECHO(rdevpoints);
  ECHO(n_deviations);
  AddcvToSurv.allocate(1,n_addcv_par);
  AddcvType.allocate(1,n_addcv_par);
  int iaddcv; iaddcv = 0;
  for (int ii=1;ii<=nSurveys;ii++)
   if (add_cv_mirror(ii)==0)
    {
  ...
  add_cv_ival.allocate(1,n_addcv_par);
  add_cv_lb.allocate(1,n_addcv_par);
  add_cv_ub.allocate(1,n_addcv_par);
  prior_add_cv_type.allocate(1,n_addcv_par);
  prior_add_cv_p1.allocate(1,n_addcv_par);
  prior_add_cv_p2.allocate(1,n_addcv_par);
  cv_phz.allocate(1,n_addcv_par);
  add_cv_pars.allocate(1,n_addcv_par,1,7,"add_cv_pars");
 ...
 echoinput << add_cv_pars << endl;
  log_add_cv_ival.allocate(1,n_addcv_par);
  log_add_cv_lb.allocate(1,n_addcv_par);
  log_add_cv_ub.allocate(1,n_addcv_par);
 ...
 ECHOSTR(" * Fishing mortality controls");
  ...
  init_fbar.allocate(1,nfleet);
  log_init_fbar.allocate(1,nfleet);
  log_init_fbar_foff.allocate(1,nfleet);
 ...
 ECHOSTR(" * Size composition controls");
  iSizeCompType_in.allocate(1,nSizeComps_in,"iSizeCompType_in");
 for (int k =1; k<=nSizeComps_in; k++)
  ...
  bTailCompression_pmin_in.allocate(1,nSizeComps_in,"bTailCompression_pmin_in");
  iCompAggregator.allocate(1,nSizeComps_in,"iCompAggregator");
  lf_catch_in.allocate(1,nSizeComps_in,"lf_catch_in");
  lf_lambda_in.allocate(1,nSizeComps_in,"lf_lambda_in");
  lf_emphasis_in.allocate(1,nSizeComps_in,"lf_emphasis_in");
  survey_for_q_in.allocate(1,nSizeComps_in,"survey_for_q_in");
 ...
 echoinput << "nSizeComps from iCompAggregator: " << nSizeComps << endl;
  ...
  iSizeCompType.allocate(1,nSizeComps);
  bTailCompression.allocate(1,nSizeComps);
  lf_catch.allocate(1,nSizeComps);
  lf_lambda.allocate(1,nSizeComps);
  lf_emphasis.allocate(1,nSizeComps);
  log_vn_pars.allocate(1,nSizeComps,1,7,"log_vn_pars");
 echoinput << "log_vn_pars: " << endl << log_vn_pars << endl;
  log_vn_ival.allocate(1,nSizeComps);
  log_vn_lb.allocate(1,nSizeComps);
  log_vn_ub.allocate(1,nSizeComps);
  log_vn_phz.allocate(1,nSizeComps);
  prior_log_vn_type.allocate(1,nSizeComps);
  prior_log_vn_p1.allocate(1,nSizeComps);
  prior_log_vn_p2.allocate(1,nSizeComps);
  ...
  mapInpToAggSizeComps.allocate(1,nMapInpToAggSizeComps,1,6);
  ...
  TempSize.allocate(1,nclass);
  cumsum.allocate(1,nclass);
  tailcompind.allocate(1,nSizeComps_in,1,nSizeCompRows_in,1,2);
  int i,j;
 ...
 ECHOSTR(" * Tagging controls");
  tag_emphasis.allocate("tag_emphasis");
 ...
 cout << "## Emphasis Factors (Fdev Penalties) ##" << endl;
  Penalty_fdevs.allocate(1,nfleet,1,4,"Penalty_fdevs");
 ...
 gmacs_ctl << "\n## Emphasis Factors (Priors/Penalties: 13 values) ##" << endl;
  Penalty_emphasis.allocate(1,13,"Penalty_emphasis");
 ...
 if (n_deviations_est==0) n_deviations_est = 1;
  deviations_ival.allocate(1,n_deviations_est);
  deviations_phz.allocate(1,n_deviations_est);
 if (n_deviations > 0)
 {
 ...
 if (spr_syr > spr_nyr) spr_syr = spr_nyr;
  spr_SexR_syr.allocate("spr_SexR_syr");
 ...
 if (spr_SexR_syr < syr) { cout << "First year for computing sex ratio ("<<spr_SexR_syr<<") must be syr ("<<syr<<") or later: STOPPING" << endl; exit(1); }
  spr_SexR_nyr.allocate("spr_SexR_nyr");
 ...
 if (spr_M_syr > spr_M_nyr) spr_M_syr = spr_M_nyr;
  spr_Prop_syr.allocate("spr_Prop_syr");
 ...
 if (spr_Prop_syr < syr) { cout << "First year for computing season lengths (spr_Prop_syr = "<<spr_Prop_syr<<") must be syr ("<<syr<<") or later: STOPPING" << endl; exit(1); }
  spr_Prop_nyr.allocate("spr_Prop_nyr");
 ...
 if (spr_Prop_syr > spr_Prop_nyr) spr_Prop_syr = spr_Prop_nyr;
  spr_grow_yr.allocate("spr_grow_yr");
 ...
 spr_grow_yr = spr_grow_yr - nyrRetroNo;
  spr_sel_syr.allocate("spr_sel_syr");
 ...
 if (spr_sel_syr < syr) { cout << "First year for computing selex (spr_sel_syr = "<<spr_sel_syr<<") must be syr ("<<syr<<")  or later: STOPPING" << endl; exit(1); }
  spr_sel_nyr.allocate("spr_sel_nyr");
 ...
 nprojVec = nproj+1; if (nprojVec<2) nprojVec = 2;
  Project_type.allocate("Project_type");
 WRITEPRJ(Project_type);
  prj_Nstrat.allocate("prj_Nstrat");
 WRITEPRJ(prj_Nstrat);
  Proj_Levels.allocate(1,prj_Nstrat,"Proj_Levels");
 ...
 proj_nyr = proj_nyr - nyrRetroNo;
  proj_SexR_syr.allocate("proj_SexR_syr");
 ...
 if (proj_SexR_syr < syr) { cout << "First year for computing sex ratio (proj_SexR_syr="<<proj_SexR_syr<<") must be syr or later: STOPPING" << endl; exit(1); }
  proj_SexR_nyr.allocate("proj_SexR_nyr");
 ...
 if (proj_SexR_syr > proj_SexR_nyr) proj_SexR_syr = proj_SexR_nyr;
  proj_aveF_syr.allocate("proj_aveF_syr");
 ...
 if (proj_aveF_syr < syr) { cout << "First year for computing AveF (proj_aveF_syr="<<proj_aveF_syr<<") must be syr ("<<syr<<")or later: STOPPING" << endl; exit(1); }
  proj_aveF_nyr.allocate("proj_aveF_nyr");
 ...
 if (proj_aveF_syr > proj_aveF_nyr) proj_aveF_syr = proj_aveF_nyr;
  proj_M_syr.allocate("proj_M_syr");
 ...
 if (proj_M_syr < syr) { cout << "First year for computing M (proj_M_syr="<<proj_M_syr<<") must be syr or later: STOPPING" << endl; exit(1); }
  proj_M_nyr.allocate("proj_M_nyr");
 ...
 if (proj_M_syr > proj_M_nyr) proj_M_syr = proj_M_nyr;
  proj_Prop_syr.allocate("proj_Prop_syr");
 ...
 if (proj_Prop_syr < syr) { cout << "First year for computing season lengths (proj_Prop_syr="<<proj_Prop_syr<<") must be syr or later: STOPPING" << endl; exit(1); }
  proj_Prop_nyr.allocate("proj_Prop_nyr");
 ...
 if (proj_Prop_syr > proj_Prop_nyr) proj_Prop_syr = proj_Prop_nyr;
  proj_grow_yr.allocate("proj_grow_yr");
 ...
 proj_grow_yr = proj_grow_yr - nyrRetroNo;
  proj_sel_syr.allocate("proj_sel_syr");
 ...
 if (proj_sel_syr < syr) { cout << "First year for computing selex (proj_sel_syr="<<proj_sel_syr<<") must be syr or later: STOPPING" << endl; exit(1); }
  proj_sel_nyr.allocate("proj_sel_nyr");
...
if (Project_type==2) Apply_HCR_prj = 0;
 WRITEPRJ(Apply_HCR_prj);
  Apply_StateHCR_prj.allocate("Apply_StateHCR_prj");
 WRITEPRJ(Apply_StateHCR_prj);
  NHCRpars.allocate("NHCRpars");
 WRITEPRJ(NHCRpars);
  HCRpars.allocate(1,NHCRpars,"HCRpars");
 WRITEPRJ(HCRpars); 
  max_prj.allocate("max_prj");
 WRITEPRJ(max_prj);
  full_prj_diag.allocate("full_prj_diag");
...
void model_parameters::initializationfunction(void)
{
  ...
  G_pars_est.set_initial_value(G_ival);
  M_pars_est.set_initial_value(M_ival);
  S_pars_est.set_initial_value(S_ival);
  log_fbar.set_initial_value(log_init_fbar);
  log_foff.set_initial_value(log_init_fbar_foff);
  log_vn.set_initial_value(log_vn_ival);
  ...
  par_devs.set_initial_value(deviations_ival);
  if (global_datafile)
  {
    delete global_datafile;
    global_datafile = NULL;
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 ECHO(theta);
  G_pars_est.allocate(1,n_Gpar,G_lb,G_ub,G_phz,"G_pars_est");
ECHO(G_pars_est);
  M_pars_est.allocate(1,n_Mpar,M_lb,M_ub,M_phz,"M_pars_est");
ECHO(M_pars_est);
  S_pars_est.allocate(1,n_Spar,S_lb,S_ub,S_phz,"S_pars_est");
ECHO(S_pars_est);
 ECHO(f_phz);
...
ECHO(nYparams);
 ...
 ECHO(rdv_syr); ECHO(rdv_eyr);
  rec_dev_est.allocate(rdv_syr,rdv_eyr,-12.0,12.0,rdv_phz,"rec_dev_est");
 ECHO(rec_dev_est);
 ...
 ECHO(log_vn_ub);
 ECHO(log_vn_phz);
  log_vn.allocate(1,nSizeComps,log_vn_lb,log_vn_ub,log_vn_phz,"log_vn");
 ECHO(log_vn);
 ECHO(n_qpar)
  survey_q.allocate(1,n_qpar,q_lb,q_ub,q_phz,"survey_q");
 ECHO(survey_q);
ECHO(log_add_cv_ival);
  log_add_cv.allocate(1,n_addcv_par,log_add_cv_lb,log_add_cv_ub,cv_phz,"log_add_cv");
 ECHO(log_add_cv);
  par_devs.allocate(1,n_deviations_est,-12.0,12.0,deviations_phz,"par_devs");
 ECHO(par_devs);
  ...
  nloglike.allocate(1,nlikes,1,ilike_vector,"nloglike");
    ...
    nloglike.initialize();
  #endif
  nlogPenalty.allocate(1,13,"nlogPenalty");
  #ifndef NO_AD_INITIALIZE
  ...
  logN0.allocate(1,n_grp,1,nclass,"logN0");
    ...
    logN0.initialize();
  #endif
  alpha.allocate(1,nsex,1,maxSizeIncVaries,"alpha");
  #ifndef NO_AD_INITIALIZE
    alpha.initialize();
  #endif
  beta.allocate(1,nsex,1,maxSizeIncVaries,"beta");
  #ifndef NO_AD_INITIALIZE
    beta.initialize();
  #endif
  ...
  molt_cv.allocate(1,nsex,1,nMoltVaries,"molt_cv");
    ...
    molt_cv.initialize();
  #endif
  mature_mu.allocate(1,nsex,1,nMoltVaries,"mature_mu");
  #ifndef NO_AD_INITIALIZE
    mature_mu.initialize();
  #endif
  mature_cv.allocate(1,nsex,1,nMoltVaries,"mature_cv");
  #ifndef NO_AD_INITIALIZE
    mature_cv.initialize();
  #endif
  rec_pass.allocate(1,nsex,1,nclass,"rec_pass");
  #ifndef NO_AD_INITIALIZE
    rec_pass.initialize();
  #endif
  totrecruits.allocate(syr,nyrRetro,"totrecruits");
  #ifndef NO_AD_INITIALIZE
    totrecruits.initialize();
  #endif
  ...
  #endif
  molt_prob_pass.allocate(1,nsex,1,nclass,"molt_prob_pass");
  #ifndef NO_AD_INITIALIZE
    molt_prob_pass.initialize();
  #endif
  ...
  #endif
  mature_prob_pass.allocate(1,nsex,1,nclass,"mature_prob_pass");
  #ifndef NO_AD_INITIALIZE
    mature_prob_pass.initialize();
  #endif
  mature_probability.allocate(1,nsex,syr,nyr,1,nclass,"mature_probability");
  #ifndef NO_AD_INITIALIZE
    mature_probability.initialize();
  #endif
  mature_probability_in.allocate(1,nsex,1,maxSizeIncVaries,1,nclass,"mature_probability_in");
  #ifndef NO_AD_INITIALIZE
    mature_probability_in.initialize();
  #endif
  ...
  #endif
  S_pars.allocate(1,100,1,nclass,"S_pars");
  #ifndef NO_AD_INITIALIZE
    S_pars.initialize();
  #endif
  ...
  #endif
  selex_smooth_pen.allocate("selex_smooth_pen");
  #ifndef NO_AD_INITIALIZE
  selex_smooth_pen.initialize();
  #endif
  M.allocate(1,nsex,1,nmature,syr-1,nyr,1,nclass,"M");
  #ifndef NO_AD_INITIALIZE
    M.initialize();
  #endif
  ...
  S.allocate(1,nsex,1,nmature,syr,nyrRetro,1,nseason,1,nclass,1,nclass,"S");
    ...
    S.initialize();
  #endif
  SS_pass.allocate(1,nsex,1,nmature,1,nseason,1,nclass,1,nclass,"SS_pass");
  #ifndef NO_AD_INITIALIZE
    SS_pass.initialize();
  #endif
  d4_N.allocate(1,n_grp,syr,nyrRetro+1,1,nseason,1,nclass,"d4_N");
    ...
    d4_N.initialize();
  #endif
  d4_N_init.allocate(1,n_grp,1,NyrEquil,1,nseason,1,nclass,"d4_N_init");
  #ifndef NO_AD_INITIALIZE
  ...
  TempSS.initialize();
  #endif
  SurveyQT.allocate(1,nSurveys,syr,nyrRetro+1,"SurveyQT");
  #ifndef NO_AD_INITIALIZE
    SurveyQT.initialize();
  #endif
  AddVarQT.allocate(1,nSurveys,syr,nyrRetro+1,"AddVarQT");
  #ifndef NO_AD_INITIALIZE
    AddVarQT.initialize();
  #endif
  ...
  res_cpue.allocate(1,nSurveyRows,"res_cpue");
    ...
    res_cpue.initialize();
  #endif
  res_cpue_stdzd.allocate(1,nSurveyRows,"res_cpue_stdzd");
  #ifndef NO_AD_INITIALIZE
    res_cpue_stdzd.initialize();
  #endif
  ...
  #endif
  catch_summary.allocate(1,nfleet+5,1,nproj,"catch_summary");
  #ifndef NO_AD_INITIALIZE
    catch_summary.initialize();
  #endif
...
dvar_mid_points = mid_points;
  histcat.allocate(1,2+nfleet,syr,nyr,"histcat");
  #ifndef NO_AD_INITIALIZE
    histcat.initialize();
  #endif
  spr_rbar.allocate(1,2,"spr_rbar");
  #ifndef NO_AD_INITIALIZE
    spr_rbar.initialize();
  #endif
  ...
  spr_cofl.initialize();
  #endif
  spr_cofl_ret.allocate("spr_cofl_ret");
  #ifndef NO_AD_INITIALIZE
  spr_cofl_ret.initialize();
  #endif
  ...
  Bmsy.initialize();
  #endif
  proj_rbar.allocate(1,2,"proj_rbar");
  #ifndef NO_AD_INITIALIZE
    proj_rbar.initialize();
  #endif
  ...
  #endif
  numbers_proj_gytl.allocate(1,n_grp,1,nprojVec,1,nseason,1,nclass,"numbers_proj_gytl");
  #ifndef NO_AD_INITIALIZE
    numbers_proj_gytl.initialize();
  #endif
  OFLoutpass.allocate(1,2+nfleet,"OFLoutpass");
  #ifndef NO_AD_INITIALIZE
    OFLoutpass.initialize();
  #endif
  gradientOut.allocate(1,NEstPars,"gradientOut");
  #ifndef NO_AD_INITIALIZE
    gradientOut.initialize();
  #endif
  sd_log_recruits.allocate(1,nsex,syr,nyrRetro,"sd_log_recruits");
  ...
  sd_fofl.allocate(1,nfleet,"sd_fofl");
    ...
    sd_fofl.initialize();
  #endif
  sd_log_ssb.allocate(syr,nyrRetro,"sd_log_ssb");
  sd_last_ssb.allocate("sd_last_ssb");
  dyn_Bzero.allocate(syr,nyrRetro,"dyn_Bzero");
  #ifndef NO_AD_INITIALIZE
    dyn_Bzero.initialize();
  #endif
}
...
dvar_vector model_parameters::timevarparV(const int itype, const int ipar, const int lyear,  const int blk, 
...
dvar_matrix model_parameters::timevarparM(const int itype, const int ipar, const int lyear,  
...
dvar_vector model_parameters::calc_growth_increments_data(const dvector vSizes, const ivector iSex, int igrow)
{
  {
...
void model_parameters::calc_mature_probability(void)
{
  double tiny = 0.000;
  mature_probability.initialize();
  for ( int i = syr; i <= nyrRetro; i++ )
...
void model_parameters::update_population_numbers_at_length_season(const int i, const int j, const int Case)
{
...
dvar_vector model_parameters::calc_ssba()
{
  int ig,h,m,o;
...
dvar_matrix model_parameters::calc_brute_equilibrium(const int YrRefSexR1, const int YrRefSexR2, const int YrRef, const int YrRefGrow, 
...
dvar_vector model_parameters::project_biomass(const int YrRef2, const int YrRefGrow, const int YrRefM1, const int YrRefM2, const int YrRefSea1,
...
void model_parameters::project_one_year(const int i, const int iproj, const int YrRef2, const int YrRefGrow, const int YrRefM1,
...
dvar_vector model_parameters::calc_predicted_project(const int YrRef2, const int YrRefGrow, const int YrRefSel1, const int YrRefSel2,
...
dvar_vector model_parameters::compute_OFL_and_ABC(const int iyr, const int YrRefGrow, const int YrRefM1, const int YrRefM2,
...
dvariable model_parameters::project_biomass_OFL(const int iyr, const int YrRef2, const int YrRefGrow, 
        const int YrRefM1, const int YrRefM2, const int YrRefSea1, const int YrRefSea2, 
...
adstring model_parameters::strg(double d)
{
      char  buffer[50];
      sprintf(buffer,"%g",d);
      adstring tmp(buffer);
...
adstring model_parameters::to_str(const dvector& v)
{
      int g=1;
      int mn = v.indexmin();
      int mx = v.indexmax();
...
adstring model_parameters::to_str(const dvar_vector& v)
{
      dvector vp = value(v);
...
void model_parameters::WriteFiles(void)
{
  OutInpFile1.close();
...
void model_parameters::set_runtime(void)
{
  ...
  dvector temp("{1.e-2, 1.e-2, 1.e-3, 1.e-7, 1.e-7}");
...
dvariable model_parameters::CalcStateTAC(const int i, const int iproj,const int YrRefGrow)
{
 dvariable StateTAC;
 // Very large TAC
 StateTAC = 1.0e20; 
...
dvariable model_parameters::StMatsTAC(const int j, const int iproj,const int YrRefGrow)
{
  double lam;
  int isizeTrans; 
