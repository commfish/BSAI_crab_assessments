...
2. **16.0 - 2020 Reference Model**: updated with 2019/20 groudfish bycatch 

3. **16.0a - 2020 Reference Model with fixed terminal year recruitment**: terminal year recruitment fixed as the average of the last seven years
...
was acceptable at 1.66 in the reference model. Francis (2011) weighting was applied in 2017 but given the relatively few size bins in this assessment, this application was suspended for this assessment.  
...
The parameter estimates in the "no pot" scenario differ greatly from the reference model, as expected, due to the removal of recent ADF&G pot survey data points that pulled the MMB trend downward (Table \@ref(tab:est-pars-all)). 
...
The reference model fit to total male ($\ge$ 90 mm CL) trawl survey biomass tends to miss the recent peak around 2010 and fits recent survey data points on the lower end of their error bars (Figures \@ref(fig:trawl-survey-biomass)). 
...
The reference or base model fit to the pot survey CPUE is similar to past reference models, fitting the overall trends in the data but not capturing some of the high and low points (Figure \@ref(fig:pot-survey-cpue)). 

For the trawl survey the standardized residuals are more balanced in model 20.1 (**no pot**), without the ADF&G pot survey data, especially in recent years. The reference model has a clear residual pattern in the last 15 years, continually under predicting the observed data points (Figure \@ref(fig:bts-resid-nmfs)). The standardized residuals for the ADF&G pot survey have similar patterns to past reference model iterations (Figure \@ref(fig:bts-resid-adfg)).
...
In summary, the **no pot** model scenario was provided to explore the sensitivity of this model. Currently, the reference model is still the most appropriate model for settting reference points and model specifications. 
...
The two reference models presented here, 16.0 and 16.0a, only differ in the estimation of 2019 recruitment. 
Model 16.0a fixes the 2019 recruitment to be the average of the last seven years of the model, effectively limiting the model's ability to estimate unreasonably high recruitment in the lack of a 2020 data point. However, fixing terminal year recruitment has a minimal effect on the status of the stock, projected MMB, or the resulting OFL for 2020 (Table \@ref(tab:management-quants)). The recommended model for 2020 would be the reference model (16.0) to maintain consistency for this stock during the rebuilding time frame and with the lack of a 2020 data point for the trawl survey. 
...
df1 %>% 
  bind_rows(df2) -> df
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the reference model with fixed terminal year recruitment 'fixR' (16.0a).", label = "tab:est-pars-fixR", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
names(df) <- c("Year","$n_1$","$n_2$","$n_3$","MMB","CV MMB")
tab <- xtable(df, digits = c(0,0,0,0,0,0,3), caption = "Population abundances ($\\boldsymbol{n}$) by crab stage in numbers of crab at the time of the survey (1 July, season 1) and mature male biomass (MMB) in tons on 15 February for the 2020 reference model.", label = "tab:pop-abundance-base")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function=function(x){x})
```
...
```{r surv1, fig.cap = "Fits to NMFS area-swept trawl estimates of total (> 90mm) male survey biomass for the reference model only (16.0 ref for 2020 and 16.0 2019 accepted model). Error bars are plus and minus 2 standard deviations."} 
knitr::include_graphics("safe_figure/trawl_cpue_ref.png", dpi = 200)
```


```{r surv2, fig.cap = "Comparisons of fits to CPUE from the ADFG pot surveys for the reference model 16.0 reference model in 2019 and 2020. Error bars are plus and minus 2 standard deviations."} 
...
```{r fishing-mort, fig.cap = "Fishing mortality estimates from the reference model (16.0) for directed and bycatch fleets", fig.height =12}
