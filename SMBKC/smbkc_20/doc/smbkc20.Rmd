...
The base model presented here is the reference model with updated groundfish bycatch data for the 2019/20 crab season (model 16.0 base). One additional model is presented for consideration, which is a small variant of the base model, model 16.0a, which fixes recruitment in the most recent year to the average of the last few years to avoid unrealistically high recruitment estimates. Additionally, further simulations and analysis were done to assess the uncertainty in the 2020 biomass estimates and reference point calculations (Appendix C). 
...
Comment: *Regarding general code development, the SSC and CPT outstanding requests continue to be as follows:*
  ...
  1. *add the ability to conduct retrospective analyses*
     ...
     Retrospective runs/simulations are presented here in Appendix C as part of the analyses done to assess uncertainty in the model output. The ability to automate these in GMACS is still under developement but the author was able to do them by manually editing the data files.
...
for both scenarios. 
Representative residual plots of the composition data fits are show generally poor fit to the three composition data sources (Figures \@ref(fig:sc-res-ref), \@ref(fig:sc-res-fixR) and \@ref(fig:sc-res-nopot).
...
The estimates of mature male biomass (Figure \@ref(fig:mmb)) for the **no pot** model differs from the reference models (16.0 and 16.0a) due to the removal of the pot survey abundance and size composition data. This abundance time series contrasts with the NMFS trawl survey and when present tends to lower the scale of the population estimate. 
...
df4 <- data.frame(Component = c(paste0("$\\text{OFL}_{", (M[[2]]$mod_yrs[length(M[[2]]$mod_yrs)]+ 1), "}$"),
                  ...
                  Ref = c(ref_ofl$OFL_2020, ref_ofl$OFL_2020*.80) , 
                  fixR = c(fixR_ofl$OFL_2020, fixR_ofl$OFL_2020*.80) ,
                  nopot = c(nopot_ofl$OFL_2020, nopot_ofl$OFL_2020*.80))
df %>% 
  rbind(df4) -> df2
