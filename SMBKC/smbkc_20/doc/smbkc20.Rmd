...
df1 %>% 
  bind_rows(df2) -> df
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the reference model with fixed terminal year recruitment (16.0a).", label = "tab:est-pars-fixR", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
```{r retrospective, fig.cap = "Retrospective pattern in mature male biomass(mmb, t) for the reference (base) model.", fig.height =12}
