...
## The reference model (16.0) data file for 2020 {-}
...
ts <- readLines(here::here("SMBKC/smbkc_20/data/sm20.dat"))
for (i in 1:length(ts)) cat(ts[i], "\n")
```
## The reference model (16.0) control file for 2020{-}
```{r control, eval = TRUE, include=TRUE,echo=FALSE, comment=NA}
opts_chunk$set(fig.width = 12, fig.height = 8, echo = FALSE, warning = FALSE, message = FALSE)
ts <- readLines(here::here("SMBKC/smbkc_20/model_1/sm20.ctl"))
for (i in 1:length(ts)) cat(ts[i], "\n")
```
