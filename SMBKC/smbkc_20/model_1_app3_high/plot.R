...
# Model: model_1_app3_high - model with fake 2020 survey data based on fit app3a and multiplicative quantiles 
...
mod_names <- c("2020 base - App 3 high")
.MODELDIR = c("./SMBKC/smbkc_20/model_1_app3_high/") # directory where the model results are
...
.SEAS     = c("Annual")
.FIGS     = c("./SMBKC/smbkc_20/model_1_app3_high/figure/")
...
write.csv(ssb, paste0(.FIGS, paste0("ssb_", cur_yr, "_app3_high.csv")), row.names = FALSE)
...
rec %>% 
  ...
  write.csv(paste0(.FIGS, paste0("recruitment_output_", cur_yr, "_app3_high.csv"))) 
...
temp <- data.frame(year = cur_yr, 
                   ...
                   type = "App3_H")
temp
write.csv(temp, paste0(.FIGS, "summary_app3_high.csv"), row.names = FALSE)
