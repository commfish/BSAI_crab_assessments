...
In addition to the reference/base model other models are presented here for sensitivity of life history constants. Here two models are presented that raise the natural mortality from the accepted fixed value of 0.18 for king crab. Model 22.0a has a M = 0.21,  for considerations, one additional model is presented here to assess sensitivity of data inputs to the model, attempting to deal with the disparity between the two survey time series (**no pot**). The **no pot** configuration runs the base model 16.0 without the ADF&G pot survey data, therefore only having the NMFS trawl survey as the abundance index. 
