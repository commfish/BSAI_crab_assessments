...
1
## number of size-increment periods (time blocking of size transition matrix)
1
## Two lines for each parameter if split sex, one line if not                           ##
## number of molt periods
1
## Year(s) molt period changes (blank if no changes)
## Beta parameters are relative (1=Yes;0=no) (doesn't matter here because not estimated)
1
## ==================================================================================== ##
# ival        lb        ub         phz  prior     p1      p2         # parameter         #
...
## Gear-1    Gear-2   Gear-3    Gear-4   Gear-5
   ...
   0         3        3         0        0         # male selectivity type (why am I using 0 here??)
   0         0        0         0        0         # within another gear
## Gear-1    Gear-2   Gear-3    Gear-4   Gear-5
   1         1        1         1        1         # Retention periods
   0         0        0         0        0         # sex specific retention (this is wrong. males retained females discarded)
...
## ==================================================================================== ##
## Length Comps matrices from .dat file (line 260)
...
#  5   5   5   # Type of likelihood
 ...
 -4  -4  -4   # Phz for estimating effective sample size (if appl.)
  1   2   3   # Composition aggregator appended not really aggregated (combines males and females)
  1   1   1   # LAMBDA (if want to remove sample group, i.e. length comps from groundfish, set lambda =0)
  1   1   1   # Emphasis
## ==================================================================================== ##
...
##      3 = Blocked changes (deviates constrained by variance at specific knots, not back to base value)
##      4 = Time blocks
##      5 = Like 3 but back to base instead of newly estimated M
##      6 = Like 4 but back to base instead of new estimated M
## ==================================================================================== ##
## Type
6
...
10.0
## Number of nodes for cubic spline or number of step-changes for option 3 (has two values if males and females)
2
...
1
## =========================================================================================== ##
## ival        lb        ub        phz   extra(mirrors parameters)    prior     p1      p2         # parameter     ##
## =========================================================================================== ##
...
2018       # last rec_dev
  ...
  -3       # Estimated rec_ini phase
   ...
   2       # Initial conditions (0 = Unfished, 1 = Steady-state fished, 2 = Free parameters, 3 = Free parameters(revised -where you estimate everything minus 1))
...
## ==================================================================================== ##
# Log_fdevs   meanF       Mdevs  Rec_devs Initial_devs Fst_dif_dev Mean_sex_recruitment-Ratio
      10000       1           1         1            0           0             1             #(10000)
### 10000 here ensures that the mena of the fdevs = 0
### if 0 the likelihood component goes away 
### if you want mean_sex_ratio to be the same make this large
## EOF
9999
