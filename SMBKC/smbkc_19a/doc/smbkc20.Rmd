...
(i.e., the "dynamic $B_0$") suggests, that the current spawning stock biomass has been reduced to `r .get_dynB0_df(Mbase) %>% transmute(round(100*ssb,0)) %>% slice(n())`% of what it would have been in the absence of fishing, assuming the same level of recruitment as estimated. 
 ...
 \begin{tabular}{lccccccc}
  ...
  2020/21 &      & `r mmb_1920a` &  &  &  & `r ofl_1920a` & `r abc_1920a` \\
 \\
  \hline
\end{tabular}
\end{table}
...
1. **16.0 - 2019 Model**: the 2019 recommended model

2. **16.0 - 2019 Reference Model**: updated with Jan 2020 updates to GMACS, 

3. **19.1 - VAST NMFS trawl data**: model 16.0 with VAST data output for the NMFS trawl survey time series 

4. **19.2 - add CV pot**: model 16.0 + an estimated additional CV on the ADF&G pot survey

5. **19.3 - add CV both**: model 16.0 + an estimated additional CV on the ADF&G pot survey and the NOAA trawl survey 

6. **19.4 - q time block pot**: time block estimated q's for ADF&G pot survey. 2 time blocks: 1995 to 2013 and 2015 to 2018. 
...
**update** Results for scenarios are provided with comparisons to the 2019 model and sensitivity to new 2019 data are shown in 
...
This stock was declared overfished in fall of 2018 and a rebuilding plan was under initial review by the NPFMC in December 2019. The rebuilding plan is scheduled for final review fothe June 2020 NPFMC meeting. Model scenarios presented here all suggest the stock is still overfished.
...
knitr::include_graphics("safe_figure/trawl_cpue_ref.png", dpi = 100)
```

```{r surv2, fig.cap = "Comparisons of fits to CPUE from the ADF&G pot surveys for the reference model 16.0 ref (note that there is no new pot data for 2019). Error bars are plus and minus 2 standard deviations."} 
knitr::include_graphics("safe_figure/pot_cpue_ref.png", dpi = 100)
```
...
```{r sc-res-ref, fig.cap = "Bubble plots of residuals by stage and year for the all the size composition data sets (ADF&G pot survey, NMFS trawl survey, and the directed pot fishery) for SMBKC in the **reference** model (16.0)."}
knitr::include_graphics("safe_figure/ref_mod_size_comp_residuals.png", dpi = 100)
```

```{r sc-res-vast, fig.cap = "Bubble plots of residuals by stage and year for the all the size composition data sets (ADF&G pot survey, NMFS trawl survey, and the directed pot fishery) for SMBKC in the **VAST** model (19.1)."}
...
```{r sc-res-addcvboth, fig.cap = "Bubble plots of residuals by stage and year for the all the size composition data sets (ADF&G pot survey, NMFS trawl survey, and the directed pot fishery) for SMBKC in the **add CV both** model (19.3)."}
knitr::include_graphics("safe_figure/addCVBoth_size_comp_residuals.png", dpi = 100)
```

```{r sc-res-qblock, fig.cap = "Bubble plots of residuals by stage and year for the all the size composition data sets (ADF&G pot survey, NMFS trawl survey, and the directed pot fishery) for SMBKC in the **q time block pot** model (19.4)."}
