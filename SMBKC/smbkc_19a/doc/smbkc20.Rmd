...
The VAST model (19.1) may be an option to consider for model spefications this fall, if the CPT finds the VAST data to be appropriate for model options. In general this model fits the trawl survey data at higher biomasses throughout time, specifically higher in the late 90s (\@ref(fig:surv1-vast) and \@ref(fig:trawl_biomass_base_vast)). 
Additionally, recruitment estimates for the VAST model trend similarly to the reference model, only at a higher magnitude (\@ref(fig:recruitment2)). As stated above mature male biomass for the VAST model are also generally higher than the reference model, but tend to display a similar trend (\@ref(fig:mmb2)). 
...
```{r surv1-vast, fig.cap = "Fits to NMFS area-swept trawl estimates of total (> 90mm) male survey biomass for the VAST model only, data plotted here are VAST trawl survey estimates. Error bars are plus and minus 2 standard deviations."} 
