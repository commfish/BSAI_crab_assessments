...
historical analyses relative to fisheries impacts are limited. Completion of a retrospective analysis is anticipated to be presented in Sept. 2020.
...
df1 %>% 
  bind_rows(df2) -> df
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the 'add CV both' (19.3) model.", label = "tab:est-pars-4", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
df1 %>% 
  bind_rows(df2) -> df
tab <- xtable(df, caption = "Model parameter estimates, selected derived quantities, and their standard deviations (SD) for the 'q time block pot' (19.4) model.", label = "tab:est-pars-4", digits = 3)
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function = function(x){x})
```
...
```{r pot-add-cv, fig.cap = "Comparisons of total (90+ mm CL) male pot survey CPUEs and model predictions for the 'add CV pot' (Model 19.2) and 'add CV both' (Model 19.3) scenarios. The black error bars are plus and minus 2 standard deviations, while the red ones incorporate the additional variability ."}
