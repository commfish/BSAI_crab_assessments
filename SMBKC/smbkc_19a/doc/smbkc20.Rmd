...
\begin{tabular}{lccccccc}
  ...
  2020/21 &       & `r mmb_1920_lba` &  &  &  & `r ofl_1920_lba` & `r abc_1920_lba` \\
 \\
  \hline
\end{tabular}
\end{table}
...
\begin{tabular}{lcccccccc}
  ...
  2020/21 &    & `r bmsy_cura` & `r mmb_1920a` & `r ratio_bmsya` & 
  `r fofla ` & 1 & 1978-2018 & 0.18 \\
  \hline
\end{tabular}
\end{table}
...
Both surveys indicate a decline over the past few years. The "reference" model is that which was selected for use in 2018 and 2019.
There is only one new data set to be included this year so this becomes the updated reference model (model 16.0 ref). 
Two alternative models are presented to assess sensitivity to the model, while another is provided for alternative reference point calculations (Table \@ref(tab:ofl-basis)) using a recent regime time frame. The **fit survey** configuration simply adds emphasis on the design-based survey data (by assuming a lower input variance). The **add CV pot** configuration estimates an additional CV on the pot survey data, which in turn allows the model to fit the trawl-survey estimates better. **update here**
...
Comment: *Regarding general code development, the SSC and CPT outstanding requests continue to be as follows:*
  ...
  1. *add the ability to conduct retrospective analyses*
     ...
     Underway but progress was limited in implementing this feature. We plan to  conduct a retrospective analysis for the final assessment in September 2020.
...
Comment: *Regarding potential model scenarios for 2020, the SSC and CPT requests are:*
  ...
  Underway but progress was limited due to unexpected world events.
...
Comment: *Explore potential explanations for the discrepancy in the time trends of the two types of survey data, including movement hypotheses using spatial models (not necessarily VAST)*
  ...
  Limited progress due to data availability (ADF&G), hope to have progress update Sept. 2020
...
Four models are presented with the reference model being the same configuration as approved last year (Palof et al. 2019), **update**
...
5. **19.3 - add CV pot & trawl**: includes an estimated additional CV on the ADF&G pot survey and the NOAA trawl survey 
