...
.SEAS     = c("Annual")
.FIGS     = c("./SMBKC/smbkc_19/model_1a/compare/")
...
ssb_last <- data.frame("Model" = mod_names,"year" = c(cur_yr, cur_yr), 
                       "ssb" = c((M[[1]]$spr_bmsy * M[[1]]$spr_depl), (M[[2]]$spr_bmsy * M[[2]]$spr_depl)), 
                       "lb" = c(M[[1]]$spr_bmsy * M[[1]]$spr_depl, M[[1]]$spr_bmsy * M[[1]]$spr_depl),
                       "ub" = c(M[[1]]$spr_bmsy * M[[1]]$spr_depl, M[[1]]$spr_bmsy * M[[1]]$spr_depl))
...
ssb %>% 
  ggplot(aes(x = year, y = ssb, col = Model)) +
    ...
    ggtitle("compare model 19.0 and 19.0a") +
...
# recruitment plot ----------
rec %>% 
  ggplot(aes(year, y = exp(log_rec)/1000000, col = Model)) +
  geom_line() +
  geom_ribbon(aes(x=year, ymax = ub/1000000, ymin = lb/1000000, fill = Model), alpha = 0.25) +
  expand_limits(y=0) +
  ggtitle("base model 2019 compare to 19.0a") +
  ...
  .THEME +
  geom_hline(data = avgR_options, aes(yintercept = meanR), color = c("blue", "blue", "red", "red"), 
