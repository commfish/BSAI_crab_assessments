# K.Palof  11-20-19/ 04-01-20/ 7-28-20 / 4-14-22
...
# Stock: SMBKC
# Year and timing: 2022 - models for may 2022
# Model: model_1 - base new data 2020 bycatch, 2021 survey
...
mod_names <- c("model 16.0 (2022 base)")
.MODELDIR = c("./SMBKC/smbkc_22/model_1_22/") # directory where the model results are
...
.SEAS     = c("Annual")
.FIGS     = c("./SMBKC/smbkc_22/model_1_22/figures/")
...
## 4-14-22 names are not output for sd of derived variables in gmacs.rep, need to move them from gmacsall.out
### ssb 2022 ------
## 2022 issues with SSB sd output in gmacs.std file
ssb <- .get_ssb_dfKP(M)
ssb %>% 
  ggplot(aes(year, ssb)) +
  geom_line() +
...
plot_ssb(M) # !!! NOT working !!! doesn't show projection year, which is current year
