...
43% of the long term mean  at 5.22 million pounds (2,366t with a CV of 50%), and an 23% increase from the 2021 biomass.
...
This assessment uses the General Model for Alaska Crab Stocks (GMACS) framework since 2016. The
...
Alternative models were considered in May 2022 but none were deemed candidates for fall 2022 specfications, therefore the "reference" model is the only one presented here.
...
Comment: *Explore potential explanations for the discrepancy in the time trends of the two types of survey data, including movement hypotheses using spatial models (not necessarily VAST)*
  ...
  Exploration of the spatial extent and density differences between the two surveys (NMFS and ADF&G) was done on all male crab ($\ge$ 90 mm CL) and all years of overlap between the two surveys included in the model for May 2022 (May 2022 documentation Appendix C). The authors plan to use this and further analyses to better characterize catchability/availability for the pot survey. 
...
2014/15-2016/17; Table \@ref(tab:stage-cpue-1)); and the NMFS groundfish-observer bycatch biomass estimates (1991/92-2021/22;
...
2. **16.0 - 2022 **: 2021 model (16.0 - 2021) updated with 2021/22 groundfish bycatch & 2022 NMFS trawl data
...
Model fit and parameter estimates are very similar to the 2021 base model runs presented in May 2022 and compared here. Selectivity estimates for the directed fishery (Figure \@ref(fig:selectivity)) as well as recruitment (Figure \@ref(fig:recruitment2) are identical with an additional year of data. 
Fits to the NMFS trawl survey data and estimated mature male biomass (MMB) on 15 February are nearly identical among the models (Figure \@ref(fig:ssb1)). 

Estimated natural mortality in each year ($M_t$) is presented in Figure \@ref(fig:M-t), 
showing the mortality event in the late 90s for all models.
Estimates of fishing mortality, from the base model (16.0 2022), are shown to assist with the rebuilding and reference point time frame discussions (Figure \@ref(fig:fishing-mort)). Fishing mortality can not be ruled out as being an influential factor in the current low stock status.

### d. Evaluation of the fit to the data.{-}
The base model (model 16.0 2022) fit to total male ($\ge$ 90 mm CL) NMFS trawl survey biomass tends to miss the recent peak around 2010 and fits recent survey data points on the lower end of their error bars (Figures \@ref(fig:trawl-survey-biomass)). 
...
The base model fit to the pot survey CPUE is similar to past reference models, fitting the overall trends in the data but not capturing some of the high and recent low points (Figure \@ref(fig:pot-survey-cpue)). 

For the trawl survey the standardized residuals for all model scenarios have a positive residual pattern in the last 16 years, continually under predicting the observed data points (Figure \@ref(fig:bts-resid-nmfs)). The standardized residuals for the ADF&G pot survey have similar patterns to past reference model iterations (Figure \@ref(fig:bts-resid-adfg)).
...
Representative residual plots of the composition data generally have a similar fit to the three composition data sources (Figures \@ref(fig:sc-res-ref)).
...
The retrospective pattern of MMB for 10 peels for model 16.0 - 2022 is shown in Figure \@ref(fig:project). The Mohn's rho value of -0.183 shows some degree of misspecification but not severe, especially for the last few years. 
...
Table \@ref(tab:est-pars-ref). Model estimates of mature male biomass and OFL in 2022/23 are presented in Section F.

 

### g. Comparison of alternative model scenarios.{-}
Currently, the base model is still the most appropriate model for setting reference points and model specifications. 
Research on alternative model specifications that may address the disparities between the trawl and pot survey data are ongoing, and a preliminary spatial overview is presented in May 2022 document Appendix C.  
...
$F_\mathit{OFL}$, OFL, ABC, and MMB in 2022/23 for the current base model are summarized in Table \@ref(tab:management-quants). The currently recommended ABC
...
tab <- xtable(df, caption = "Comparisons of management measures for the base model. Biomass and OFL are in tons.", label = "tab:management-quants", digits = 3)
...
This stock was declared overfished in fall of 2018 and a rebuilding plan was approved by the NPFMC Council in June 2020. The most updated rebuilding plan can be found on the NPFMC website for the June 2020 meeting. 
This assessment was moved to a biannual assessment in early 2021, with full assessments performed in even numbered years which falls in line with the two year rebuilding progress updates required under the rebuilding plan. 

The recovery of this stock is highly dependent upon successful recruitment, which is likely linked to climate variability but not well understood. Survey biomass of males in the model has been low in 2021 and 2022, although both increased slightly from the low level in 2018 (Figure \@ref(fig:surv1). Estimated MMB increased slightly in 2022, mostly due to an increase in recruitment and perhaps low fishing mortality on mature biomass. Model estimates of recruitment increased both in 2021 and 2022 suggesting some potential for future stock growth (Figures \@ref(fig:recruitment2), \@ref(fig:project) and \@ref(fig:project2)).
Projections of the stock tens years into the future, under two potential recruitment periods, suggest stock growth under no directed fishery pressure. (Figures \@ref(fig:project) and \@ref(fig:project2)). However, projections using a recent low recruitment period (1996 to 2021) have much more limited population growth than those using the entire recruitment time series to draw from. The 2022 ADF&G pot survey results, expected later this year, will aid in understanding the current status of this stock and its future trajectory. 
...
tab <- xtable(df, digits = c(0,0,0,0,0,0,3), caption = "Population abundances ($\\boldsymbol{n}$) by crab stage in numbers of crab at the time of the survey and mature male biomass (MMB) in tons on 15 February for the {\\bf model configuration used in 2021}.", label = "tab:pop-abundance-lastyr")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function=function(x){x})
```
...
names(df) <- c("Year","$n_1$","$n_2$","$n_3$","MMB","CV MMB")
tab <- xtable(df, digits = c(0,0,0,0,0,0,3), caption = "Population abundances ($\\boldsymbol{n}$) by crab stage in numbers of crab at the time of the survey (1 July, season 1) and mature male biomass (MMB) in tons on 15 February for the 2022 base model.", label = "tab:pop-abundance-base")
print(tab, caption.placement = "top", include.rownames = FALSE, sanitize.text.function=function(x){x})
```
...
```{r catch181, fig.cap = "Catches (in numbers) of male blue king crab > 90mm CL from the 2013-2022 NMFS trawl-survey at the 56 stations used to assess the SMBKC stock."}
knitr::include_graphics("safe_figure/CrabN_Station.png", dpi = 400)
```

```{r surv1, fig.cap = "Fits to NMFS area-swept trawl estimates of total (> 90mm) male survey biomass for the base model only (16.0 ref for 2022 and 16.0 2021 accepted model). Error bars are plus and minus 2 standard deviations."} 
knitr::include_graphics("safe_figure/trawl_cpue_ref.png", dpi = 200)
```


```{r surv2, fig.cap = "Comparisons of fits to CPUE from the ADFG pot surveys for model 16.0 the reference model in 2021 and 2022. Error bars are plus and minus 2 standard deviations."} 
...
```{r recruitment2, fig.cap = "Estimated recruitment 1979-2021 comparing ref model (16.0) for 2021 and 2022. The solid horizontal lines in the background represent the estimate of the average recruitment parameter ($\\bar{R}$) in each model scenario."}
knitr::include_graphics("safe_figure/recruitment_ref_ribbons.png", dpi = 100)
```


```{r ssb1, fig.cap = "Sensitivity of new data in 2022 on estimated mature male biomass (MMB); 1978-2022."}
...
```{r selectivity, fig.cap = "Comparisons of the estimated stage-1 and stage-2 selectivities for the different model scenarios (the stage-3 selectivities are all fixed at 1). Estimated selectivities are shown for the directed pot fishery, the trawl bycatch fishery, the fixed bycatch fishery, the NMFS trawl survey, and the ADFG pot survey. Two selectivity periods are estimated in the directed pot fishery, from 1978-2008 and 2009-2022."}
knitr::include_graphics("safe_figure/selectivity_mod_scen.png", dpi = 150)
```


```{r recruitment, fig.cap = "Estimated recruitment 1979-2021 comparing model alternatives. The solid horizontal lines in the background represent the estimate of the average recruitment parameter ($\\bar{R}$) in each model scenario. Note the high uncertainty in recruitment in the terminal year."}
#knitr::include_graphics("safe_figure/recruitment_mod_scen_ribbons.png", dpi = 250)
```



```{r mmb, fig.cap = "Comparisons of estimated mature male biomass (MMB) time series on 15 February during 1978-2022 for each of the model scenarios."}
...
```{r sc-res-ref, fig.cap = "Bubble plots of residuals by stage and year for the all the size composition data sets (ADFG pot survey, NMFS trawl survey, and the directed pot fishery) for SMBKC in the 'base' model (16.0) 2022."}
...
```{r retrospective, fig.cap = "Retrospective pattern in mature male biomass (MMB (t)) for the reference (base) model (16.0).", fig.height =12}
knitr::include_graphics("safe_figure/ssb_retrospective_model_1.png", dpi = 375)
```

```{r project, fig.cap = "Mature male biomass (MMB) projections for the next ten years using mean recruitment (1978 - 2021) and average bycatch levels from the last 5 years.", fig.height =12}
knitr::include_graphics("safe_figure/proj_ssb_model_1.png", dpi = 375)
```

```{r project2, fig.cap = "Mature male biomass (MMB) projections for the next ten years using recent recruitment draws (1996 - 2021) and average bycatch levels from the last 5 years.", fig.height =12}
knitr::include_graphics("safe_figure/proj_ssb_model_1_recent_rec.png", dpi = 375)
```              
...
\begin{enumerate}
    ...
    \end{itemize}ee Table \@ref(tab:smbkc-fishery))
    ...
    \end{itemize}
\end{enumerate}
The proportion of natural mortality ($\tau_t$) applied during each season in the model is provided in Table \@ref(tab:m-prop), see Table \@ref(tab:smbkc-fishery) for season 2 interaction with directed fishery timing. The beginning of the year (1 July) to the date that MMB is measured (15 February) is 63% of the year. Therefore 63% of the natural mortality must be applied before the MMB is calculated. Because the timing of the fishery is different each year, $\tau_2$ varies and thus $\tau_4$ varies also.
