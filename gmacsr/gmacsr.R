...
gmacs_read_allout <- function(file, model_name = NULL, version = "2.20.14") {
  ...
  if(version == "2.20.14"){  # setup ----
    ...
    out$size_fit_summary %>% 
      mutate(lambda = out$francis_weights[.$mod_series],
             nsamp_est = exp(out$parameters$estimate[grepl("Log_vn_size_comp", out$parameters$parameter)][.$mod_series]) * nsamp_obs * lambda) -> out$size_fit_summary
